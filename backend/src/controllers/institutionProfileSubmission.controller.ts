...
async function getUserInstitution(userId: string) {
...
function buildStorageKey(submissionId: string, questionId: string, originalName: string) {
  ...
  return `institution-profile-submissions/${submissionId}/${questionId}/${unique}-${safeName}`;
}

export const institutionProfileSubmissionController = {
  async getCurrent(req: AuthRequest, res: Response) {
    try {
      const userId = req.userId!;

      // Buscar submission del usuario que tenga institutionId (no edtechCompanyId)
      const submission = await prisma.profileSubmission.findFirst({
        where: {
          userId,
          institutionId: { not: null },
          edtechCompanyId: null,
      ...
      console.error('Error fetching current institution profile submission:', error);
      return res.status(500).json({ error: 'Error fetching institution profile submission' });
    }
  },
  ...
  async getById(req: AuthRequest, res: Response) {
    try {
      ...
      if (!submission || submission.userId !== userId) {
        return res.status(404).json({ error: 'Institution profile submission not found' });
      }

      return res.json(submission);
    } catch (error: any) {
      console.error('Error fetching institution profile submission:', error);
      return res.status(500).json({ error: 'Error fetching institution profile submission' });
    }
  },
  ...
  async create(req: AuthRequest, res: Response) {
    try {
      ...
      console.error('Error creating institution profile submission:', error);
      return res.status(500).json({ error: 'Error creating institution profile submission' });
    }
  },
  ...
  async update(req: AuthRequest, res: Response) {
    try {
      ...
      console.error('Error updating institution profile submission:', error);
      return res.status(500).json({ error: 'Error updating institution profile submission' });
    }
  },
