...
function initialConsentState(): ConsentState {
  return institutionSurvey.intro.consent.reduce<ConsentState>((acc, item) => {
    acc[item.id] = false
    return acc
  }, {} as ConsentState)
}
...
function buildSummarySnapshot(
  ...
  return institutionSurvey.sections.map((section) => ({
    id: section.id,
    title: section.title,
...
function cleanupHiddenAnswers(answers: AnswersState): AnswersState {
  ...
  institutionSurvey.sections.forEach((section) => {
...
export default function Questionnaire() {
  ...
  useEffect(() => {
    async function loadSubmission() {
      ...
      setServerError(null)
      try {
        const response = await axios.get('/api/institution-profile-submissions/current')
        ...
        if (data) {
          ...
          const creation = await axios.post('/api/institution-profile-submissions', {
  ...
  async function saveSubmission(options?: {
    ...
    try {
      const response = await axios.put(
        `/api/institution-profile-submissions/${submissionId}`,
        payload,
      )
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        const response = await axios.post(
          `/api/institution-profile-submissions/${submissionId}/attachments`,
          formData,
          {
            headers: {
  ...
  async function handleDeleteAttachment(question: Question, attachmentId: string) {
    ...
    try {
      await axios.delete(
        `/api/institution-profile-submissions/${submissionId}/attachments/${attachmentId}`,
      )
  ...
  async function handleDownloadAttachment(attachmentId: string) {
    ...
    try {
      const response = await axios.get(
        `/api/institution-profile-submissions/${submissionId}/attachments/${attachmentId}/url`,
      )
  ...
  function validateIntro(): boolean {
    ...
    institutionSurvey.intro.consent.forEach((item) => {
  ...
  async function handleNext() {
    ...
    const section = institutionSurvey.sections.find((item) => item.id === currentStep.id)
    if (!section) return
    ...
    return (
      <div className="bg-white shadow-lg rounded-2xl p-8 border border-gray-100">
        <h2 className="text-2xl font-semibold mb-4 text-navy-800">
          {institutionSurvey.intro.title}
        </h2>
        <p className="text-gray-600 mb-6 leading-relaxed">
          {institutionSurvey.intro.description}
        </p>
        <div className="space-y-4">
          {institutionSurvey.intro.consent.map((item) => {
          ...
          <div key={section.id} className="bg-white shadow-lg rounded-2xl p-8 border border-gray-100">
            ...
            <div className="space-y-4">
              {section.questions.map((question) => {
                const sectionDefinition = institutionSurvey.sections.find((item) => item.id === section.id)
      ...
      <div className="space-y-6">
        <div className="bg-white shadow-lg rounded-2xl p-8 border border-gray-100">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h2 className="text-2xl font-semibold text-green-700">
                Formulario de radiografía enviado
              </h2>
              <p className="text-gray-600 mt-2">
                Tu formulario de radiografía ha sido enviado exitosamente. Puedes ver el resumen completo o modificarlo si es necesario.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          >
            ...
            <p className="text-gray-600">
              Revisa todas las respuestas que enviaste en el formulario de radiografía.
            </p>
          </button>
  ...
  const currentStep = steps[currentStepIndex]
  const currentSection = institutionSurvey.sections.find(
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      <header className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-navy-900">Radiografía de Comunidades Educativas</h1>
            <p className="text-gray-600 mt-1">
              Completa cada sección para caracterizar tu institución educativa. Puedes guardar y continuar más tarde.
            </p>
          </div>
